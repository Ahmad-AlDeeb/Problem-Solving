...
void fast() {
    ...
    ll last_id{1};
    for(ll i{}; i<n; i++) {
        ll start = ranges[i].F.F;
        ll end = ranges[i].F.S;
        ll index = ranges[i].S;

        if(rooms.empty() || rooms.top().F >= start) {
            ans[index] = last_id;
            rooms.emplace(end, last_id++);
        }
        else {
            ll first_room_id = rooms.top().S;
            ans[index] = first_room_id;

            rooms.pop();
            rooms.emplace(end,first_room_id);
        }
    }
