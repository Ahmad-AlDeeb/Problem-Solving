...
void fast() {
    ...
    while(cin >> a >> b) {
        if(!a) break;
        map<ll,bool>a_exist,b_exist;
        vector<ll>a_vec{a},b_vec{b};
        a_exist[a]=true;
        b_exist[b]=true;

        ll a_copy{a}, b_copy{b};
        while(!a_exist[b] and !b_exist[a]) {
            if(a!=1) {if(a&1) a = a*3+1; else a/=2;}
            if(b!=1) {if(b&1) b = b*3+1; else b/=2;}
        ...
        ll i{};
        for(; i<a_vec.size(); i++) {
            if(a_vec[i] == b) {a_first=true; break;}
            if(b_vec[i] == a) break;
        }
        ...
        cout << a_copy << " needs " << s1 << " steps, " << b_copy << " needs " << s2 << " steps, they meet at " << c << endl;
    }
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int main() { IOS
