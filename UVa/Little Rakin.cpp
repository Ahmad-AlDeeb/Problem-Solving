...
vector<ll> sieve(ll limit) { // 10e7 < 1s
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
map<ll,ll> p_fact2(ll x) { // 1e12 or (1e14 if you have 2s)
    map<ll,ll> p;
    ll i{2};
    for(; i*i <= x; i++) {
        while(x%i == 0) p[i]++, x /= i;
...
vector<map<ll,ll>>fib;
void build() {
    map<ll,ll> before = fib[fib.size()-2];
    map<ll,ll> last = fib.back();
...
void fast() {
    ...
    map<ll,ll> p1 = p_fact2(a);
    map<ll,ll> p2 = p_fact2(b);
    fib.pb(p1); fib.pb(p2);

    while(fib.size() <= n) build();
    map<ll,ll> last = fib.back();
    for(auto p:last)
        cout << p.F << " " << p.S << endl;
}
