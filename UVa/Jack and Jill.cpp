...
void fast() {
    ...
    while(cin >> up) {
        ...
        ll times = (int)ceil(v/b-0.000000000001);
        ld time{};
        for(ll i{}; i<times; i++) {
            ld t = sqrt(2 * l / (32.2*12));
            time += up + t + (l/p) + down;
        ...
        cout  << "Scenario " << ++scene << ":\n";
        cout << "     " << setw(l_w) << left << "up hill"            << setw(r_w) << right << up << " sec\n";
        cout << "     " << setw(l_w) << left << "well diameter"      << setw(r_w) << right << d << " in\n";
        cout << "     " << setw(l_w) << left << "water level"        << setw(r_w) << right << lvl << " in\n";
        cout << "     " << setw(l_w) << left << "bucket volume"      << setw(r_w) << right << b << " cu ft\n";
        cout << "     " << setw(l_w) << left << "bucket ascent rate" << setw(r_w) << right << p << " in/sec\n";
        cout << "     " << setw(l_w) << left << "down hill"          << setw(r_w) << right << down << " sec\n";
        cout << "     " << setw(l_w) << left << "required volume"    << setw(r_w) << right << v << " cu ft\n";
        cout << "     " << setw(l_w) << left << "TIME REQUIRED"      << setw(r_w) << right << time << " sec";
    }
}
/////////////////////////////////////////////////////////////////////////////////////
int main() { IOS
