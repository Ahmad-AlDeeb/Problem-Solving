...
void fast() {
    ...
    while(getline(cin,s1) && getline(cin,s2)) {
        ...
        vector<ll>a,b; ll num;
        while(line1 >> num) a.pb(num);
        while(line2 >> num) b.pb(num);
        sort(all(a));
        sort(all(b));
        ...
        if(a == b) cout << "A equals B" << endl;
            ...
            bool A_from_B {true};
            for(auto n:a) if(!binary_search(all(b),n)) A_from_B=false;
            if(A_from_B) {cout << "A is a proper subset of B" << endl; continue;}

            bool B_from_A {true};
            for(auto n:b) if(!binary_search(all(a),n)) B_from_A=false;
            if(B_from_A) {cout << "B is a proper subset of A" << endl; continue;}

            bool disjoint{true};
            for(auto n:a) if(binary_search(all(b),n)) disjoint=false;
            if(disjoint) {cout << "A and B are disjoint" << endl; continue;}
