...
ll calc_start_base(string n) {
    ll maxi{};
    for(auto dig:n) {
        if(isalpha(dig)) maxi = max(maxi,val[dig]);
        else             maxi = max(maxi,(ll)(dig-'0'));
    }
    return max(2LL,maxi+1);
}
...
void fast() {
    ...
    while(cin >> x >> y) {
        ...
        bool cool{false};
        ll b1 = calc_start_base(x);
        ll b2 = calc_start_base(y);

        while(b1<=36 && b2<=36) {
            ll res1 = calc_in_base(x,b1);
            ll res2 = calc_in_base(y,b2);

            if(res1 > res2) b2++;
            else if(res2 > res1) b1++;
            else {cool=true; break;}
        }

        if(cool)
            cout << x << " (base " << b1 << ") = " << y << " (base " << b2 << ")";
        else
