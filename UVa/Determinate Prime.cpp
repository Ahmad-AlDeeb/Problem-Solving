...
void add(deque<ll>&dq) {
    vector<ll>v;
    for(ll j{}; j<dq.size(); j++) v.pb(dq[j]);
    ans.pb(v);
...
void fast() {
    ...
    while(cin >> l >> r) {
        ...
        for(ll i{l}; i<=r; i++)
            if(prime(i)) {
                if(dq.size()<2) dq.pb(i), d=dq.back()-dq.front();
                else {
                    if(i-dq.back() == d) dq.pb(i);
                    else {
                        if(dq.size()>2) add(dq);
                        else dq.pop_front();
    ...
    for(auto cur:ans) {
        bool cool{true};
        for(auto nex:ans)
            if(includes(nex.begin(),nex.end(),cur.begin(),cur.end()) && cur.size()!=nex.size())
                cool = false;
        if(cool) print(cur);
    }
