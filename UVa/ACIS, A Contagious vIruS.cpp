...
void fast() {
    ll n;
    while(cin >> n && n) {
        point center; cin >> center.x >> center.y;
        vector<point> poly;
        for(ll i{}; i<n; i++) {
            ll x,y; cin >> x >> y;
        ...
        ld ans{inf}; point c;
        for(ll i{}; i<n-1; i++) {
            ld d = dist_to_seg(center,poly[i],poly[i+1],c);
            ans = min(ans,d); /// use cmp_d ?
        }
        ans = min( ans, dist_to_seg(center,poly.back(),poly.front(),c) );
