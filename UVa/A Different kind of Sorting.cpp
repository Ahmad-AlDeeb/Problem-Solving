...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
vector<ll>p;
vector<ll> sieve2(ll limit) { // 10e7 < 1s
    vector<bool> bs(limit+1,true);
//    bs.set();
    bs[0]=bs[1]=false;
...
ll p_div(ll x) { // 1e12 or (1e14 if you have 2s)
    ll cnt{};

    for (ll i{}; i < (int)p.size() && p[i]*p[i] <= x; ++i) {
...
void fast() {
    sieve2(10000000);
    vector<ll> ans(2000010); ans[1]=1;

    set<pair<ll,ll>> s;
    for(ll i{2}; i<=2000000; i++)
        s.emplace(p_div(i),i);
    ll i{2};
    for(auto num:s) ans[i++]=num.S;
