...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
vector<ll>l_pre,r_pre;
void ini_pre(ll dir, vector<ll>&v) {
    for(ll i{1}, extra{2+dir}; i<=10000000000;) {
        v.pb(i);
        i += extra;
        extra+=2;
    }
}
bool up(ll lvl, ll val) {
    if((lvl&1) and (val&1) || !(lvl&1) and !(val&1)) return true;
    return false;
}
...
void fast() {
    ...
    ll n_lvl{},m_lvl{}; bool in{false};
    for(; n_lvl < l_pre.size(); n_lvl++) {
        if(!in && l_pre[n_lvl] > m)
            m_lvl=n_lvl, in=true;
    ...
    ll steps = n_lvl - m_lvl;
    ll l_land = m + l_pre[n_lvl-1] - l_pre[m_lvl-1];
    ll r_land = m + r_pre[n_lvl-1] - r_pre[m_lvl-1];
    ...
    if(n <= l_land)
        ...
        ans += (n-r_land);
    else if(up(m_lvl,m) != up(n_lvl,n)) {
        if(up(m_lvl,m) && !up(n_lvl,n)) ans--;
        else ans++;
    }
    cout << ans << endl;
}
...
#endif
    ini_pre(-1,l_pre); ini_pre(1,r_pre);
    ll tc{1};
    cin >> tc;
    while(tc--) {
        fast();
