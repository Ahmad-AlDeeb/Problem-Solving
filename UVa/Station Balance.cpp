...
void fast() {
    ...
    while(cin >> c >> s) {
        ...
        ll sum{};
        vector<ll>spec(s); for(auto &i:spec) cin >> i, sum+=i;
        ll dummy_cnt{2*c-s};
        while(dummy_cnt--) spec.pb(0);
        sort(all(spec));

        ld IMP{}, AM = sum/(ld)c;
        cout << "Set #" << ++set << endl;
        for(ll i{},k{},j{2*c-1}; k<j; i++,k++,j--) {
            cout << " " << i << ":";
            if(spec[k]) cout << " " << spec[k];
            if(spec[j]) cout << " " <<  spec[j];
            cout << endl;
            IMP += abs(spec[k] + spec[j] - AM);
        }
