...
void fast() {
    ll n;
    while(cin >> n && n) {
        vector<pair<point,point>> stick;
        pair<point,point> s;
        while(n--) {
            cin >> s.F.x >> s.F.y >> s.S.x >> s.S.y;
            stick.pb(s);
        }
        ...
        for(auto s1:stick) {
            ...
            for(auto s2:stick) {
                ...
                if(collinear(s1.F,s1.S,s2.F) and collinear(s1.F,s1.S, s2.S) and
                    (s2.F.x>=min(s1.F.x,s1.S.x) && s2.F.x<=max(s1.F.x,s1.S.x)) &&
                    (s2.F.y>=min(s1.F.y,s1.S.y) && s2.F.y<=max(s1.F.y,s1.S.y)) or
                    (s2.S.x>=min(s1.F.x,s1.S.x) && s2.S.x<=max(s1.F.x,s1.S.x)) &&
                    (s2.S.y>=min(s1.F.y,s1.S.y) && s2.S.y<=max(s1.F.y,s1.S.y)))
                        intersect = true;

                if(intersect or
                   (CCW(s1.F,s1.S,s2.F) != CCW(s1.F,s1.S,s2.S)) and ( CCW(s2.F,s2.S,s1.F) != CCW(s2.F,s2.S,s1.S)) )
                        nails++, ground=false;
            }
            if(ground) ground_cnt++;
        }
        cout << nails/2 + ground_cnt*2 << endl;
    }
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int main() { IOS
