...
void fast() {
    ll n; cin >> n;
    vector<pair<pair<ll,ll>,ll>>jobs(n);
    for(ll i{}; i<n; i++) {
        cin >> jobs[i].F.F >> jobs[i].F.S;
        jobs[i].S = i+1;
    }

    ll rep {n};
    ll pre_days;
    while(rep--) { // bubble sort ! :D
        pre_days = 0;
        for(ll i{}; i<n-1; i++) {
            ll cur_time {jobs[i].F.F};
            ll cur_fine {jobs[i].F.S};
            ll nex_time {jobs[i+1].F.F};
            ll nex_fine {jobs[i+1].F.S};

            ll ans1 = (pre_days*cur_fine) + ((pre_days+cur_time) * nex_fine);
            ll ans2 = (pre_days*nex_fine) + ((pre_days+nex_time) * cur_fine);

            if(ans2<ans1 or (ans2==ans1 && jobs[i+1].S < jobs[i].S))
