...
///////////////////////////////////////////////// ===== Solution ===== /////////////////////////////////////////////////
ld distance(ll step) {
    return sqrt(pow(step,2)/2);
}
void fast() {
    ll MAP{1}; string s;
    while(getline(cin, s) && s!="END") {
        vll steps; vector<string> dir;
        ...
        for(ll i{}; i<sz; i++) {
            ...
            if(str=="NE") x+=distance(step), y+=distance(step);
            if(str=="SE") x+=distance(step), y-=distance(step);
            if(str=="SW") x-=distance(step), y-=distance(step);
            if(str=="NW") x-=distance(step), y+=distance(step);
        }
        cout << fixed << setprecision(3);
        cout << "Map #" << MAP++ << endl;
        cout << "The treasure is located at (" << x << "," << y << ").\n";
        cout << "The distance to the treasure is " << dist(point(0,0),point(x,y)) << ".\n\n";
    }
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int main() { IOS
