...
string fast() {
    string a; cin >> a;
    ll odd{true};
    if(a.size()%2 == 0) odd=false;

    string s; bool left{false}; ll step{1};
    ll i = a.size()/2;
    if(odd) s += a[i];
    else {
        ll l{i-1}, r{i};
        if(a[r]=='1' || a[l]=='0') s += a[r], i=r;
        else s += a[l], i=l, left=!left;
    }
    odd=!odd;
    ...
    while(s.size()<a.size()) {
        if(odd) {
            ...
            if(left) {
                l = i-1;
                r = i+step;
                if(a[l]=='1' || a[r]=='0') s += a[l], i=l;
                else s += a[r], i=r, left=!left;
            }
            else {
                l = i-step;
                r = i+1;
...
int main() { IOS
    ...
    for(ll i{1}; i<=tc; i++) {
        string res = fast();
        ll ans{};
        for(ll j= res.size()-1,exp{}; j>=0; j--,exp++)
            if(res[j]-'0') ans+=big_pow(2LL,exp), ans%=M;
