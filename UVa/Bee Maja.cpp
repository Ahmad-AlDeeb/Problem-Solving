...
void fast() {
    ll x;
    vector<string>pattern {"U","L","DL","D","R","UR"};
    while(cin >> x) {
        if(x==1) {cout << "0 0" << endl; continue;}


        ll i{},j{1},rep{1},loc{2};
        for(ll dir{}; loc<x; dir++,dir%=6) {
            if(pattern[dir] == "U")
                for(ll cnt{}; cnt<rep; cnt++) {
                    i--;
                    loc++;
                    if(loc == x) goto out;
                }

            else if(pattern[dir] == "L")
                for(ll cnt{}; cnt<rep; cnt++) {
                    j--;
                    loc++;
                    if(loc == x) goto out;
                }
            else if(pattern[dir] == "DL")
                for(ll cnt{}; cnt<rep; cnt++) {
                    i++; j--;
                    loc++;
                    if(loc == x) goto out;
                }
            else if(pattern[dir] == "D")
                for(ll cnt{}; cnt<rep; cnt++) {
                    i++;
                    loc++;
                    if(loc == x) goto out;
                }
            else if(pattern[dir] == "R") {
                for(ll cnt{}; cnt<rep+1; cnt++) {
                    j++;
                    loc++;
                    if(loc == x) goto out;
                }
