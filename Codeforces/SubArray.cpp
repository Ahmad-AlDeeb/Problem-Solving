...
void fast() {
    ...
    map<ll,set<ll>>s;
    for(ll i{}; i<k; i++) {
        if(f[vec[i]]) s[f[vec[i]]].erase(vec[i]);
        f[vec[i]]++;
        s[f[vec[i]]].insert(vec[i]);
    }

    ll ans = *(s.rbegin()->S.rbegin());
    for(ll l{},r{k}; r<n; l++,r++) {
        s[f[vec[l]]].erase(vec[l]);
        if(s[f[vec[l]]].empty())
            s.erase(s.lower_bound(f[vec[l]]));
        f[vec[l]]--;
        if(f[vec[l]]) s[f[vec[l]]].insert(vec[l]);

        if(f[vec[r]]) {
            s[f[vec[r]]].erase(vec[r]);
            if(s[f[vec[r]]].empty())
                s.erase(s.lower_bound(f[vec[r]]));
        }
        f[vec[r]]++;
        s[f[vec[r]]].insert(vec[r]);


        ans += *(s.rbegin()->S.rbegin());
    }
    cout << ans;
}
/////////////////////////////////////////////////////////////////////////////////////
