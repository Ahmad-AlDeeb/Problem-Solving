...
/////////////////////////////////////////////////////////////////////////////////////
ll n,k;
vector<ll>need(100005);
vector<ll>have(100005);

bool valid(ll x) {
    ll magic {k};
    for(ll i{1}; i<=n; i++) {
        if(have[i] >= need[i]*x) continue;
        if(have[i]+magic < need[i]*x) return false;
        magic -= (need[i]*x) - have[i];
    }
    return true;
}
...
void fast() {
    ...
    for(ll i{1}; i<=n; i++) cin >> have[i];

    ll ans{}, left{1}, right{10000000000};
    while(left <= right) {
        ll mid = left + (right-left)/2;
        if(valid(mid)) left=mid+1, ans=mid;
        else           right=mid-1;
    }
    cout << ans;
}
