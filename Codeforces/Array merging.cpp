...
void fast() {
    ...
    vector<ll>mx_f1(2*n+1),mx_f2(2*n+1);

    for(ll i{}; i<n; i++) {
        cin >> a[i];
        f1[a[i]]++;
        if(i and a[i-1]!=a[i]) {
            mx_f1[a[i-1]] = max(mx_f1[a[i-1]], f1[a[i-1]]);
            f1[a[i-1]]=0;
        }
    }
    mx_f1[a.back()] = max(mx_f1[a.back()], f1[a.back()]);

    for(ll i{}; i<n; i++) {
        cin >> b[i];
        f2[b[i]]++;
        if(i and b[i-1]!=b[i]) {
            mx_f2[b[i-1]] = max(mx_f2[b[i-1]], f2[b[i-1]]);
            f2[b[i-1]]=0;
        }
    }
    mx_f2[b.back()] = max(mx_f2[b.back()], f2[b.back()]);

    ll mx{};
    for(ll i{}; i<f1.size(); i++)
        mx = max(mx,mx_f1[i]+mx_f2[i]);
    cout << mx;
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int main() { IOS
