...
/////////////////////////////////////////////////////////////////////////////////////
ld k;
bool valid(ld x, vector<ld> vec) {
    for(ll i{},j=vec.size()-1; i<j;) {
        int res = comp_double(vec[i],x);
        if(res==1 || res==-1) break;

        ld d = x-vec[i];
        ld needed = 100*d/(100-k);
        ld extra = vec[j]-x;

        int res2 = comp_double(extra,needed);
        if(res2==1 || res2==-1) {
            vec[j]-=needed;
            vec[i++] = x;
        }
        else {
            vec[i] += extra - (extra*k/100);
            vec[j--] = x;
        }
    }

    for(ll i{}; i<vec.size(); i++)
        if(!comp_double(vec[i],x)) return false;
    return true;
}
void fast() {
    ll n; cin >> n >> k;
    ...
    ld ans{}, left{}, right{sum/vec.size()};
    while(fabs(right-left) > 10e-10) {
        ld mid = (right+left)/2;
        if(valid(mid,vec)) left=mid, ans=mid;
        else               right=mid;
    }
