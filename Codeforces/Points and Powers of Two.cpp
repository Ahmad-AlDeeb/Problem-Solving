...
void fast() {
    ...
    vector<ll>v{vec.front()};
    for(ll i{}; i<n-1; i++)
        for(ll p{}; p<=32; p++) {
            ll d = pow(2LL,p);
            if(binary_search(all(vec),vec[i]+d)) {
                v.clear();
                v.pb(vec[i]);
                v.pb(vec[i]+d);

                ll loc = distance(vec.begin(), lower_bound(all(vec),vec[i]+d));
                if(binary_search(vec.begin()+loc+1, vec.end(), vec[i]+d+d)) {
                    v.pb(vec[i]+d+d);
                    goto out;
                }
            }
