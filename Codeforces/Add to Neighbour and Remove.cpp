...
void fast() {
    ll n; cin >> n;
    vector<ll>pre(n); for(auto &i:pre) cin >> i;
    for(ll i{1}; i<n; i++)
        pre[i] += pre[i-1];

    ll tot {pre[n-1]};
    vector<ll> div = get_divisors(tot);
    sort(all(div),greater<>());

    ll best{};
    for(auto d:div) {
        ll loc{-1}; bool cool{true};
        for(ll i{}; i<pre.size(); i++) {
            if(loc==-1) {
                if(pre[i]==tot/d) loc=i;
                else if(pre[i] > tot/d) {cool=false; break;}
            }
            else {
                if(pre[i]-pre[loc] == tot/d) loc=i;
                else if(pre[i]-pre[loc] > tot/d) {cool=false; break;}
            }
        }
        if(cool) {best=d; break;}
    }
    cout << n-best;
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
