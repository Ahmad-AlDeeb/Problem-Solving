...
void fast() {
    ...
    vector<ll> b(n*m);
    for(ll i{}; i<n*m; i++) cin >> b[i];
    sort(all(b));

    ll mx = b.back();
    ll ans1 = abs(mx - b.front()) * (n * m - 1);
    if(b[0] != b[1]) {
        ll min1 = min(b[0], b[1]);
        ll min2 = min(n,m);
        ...
        ans1 -= abs(mx - min1) * (min2 - 1);
        ans1 += abs(mx - mx1) * (min2 - 1);
    }

    reverse(all(b));
    mx = b.back();
    ll ans2 = abs(mx - b.front()) * (n * m - 1);
    if(b[0] != b[1]) {
        ll min1 = min(b[0], b[1]);
        ll min2 = min(n,m);
        ...
        ans2 += abs(mx - min1) * (min2 - 1);
        ans2 -= abs(mx - mx1) * (min2 - 1);
    }
    cout << max(ans1,ans2);
