...
/////////////////////////////////////////////////////////////////////////////////////
ll n,k,a,m;
vector<ll> shots(200009);
bool valid(ll x) {
    vector<ll> sea(200009);
    for(ll i{1}; i<=x; i++)
        sea[shots[i]] = 1;
...
void fast() {
    cin >> n >> k >> a >> m;
    for(ll i{1}; i<=m; i++) cin >> shots[i];

    ll ans{-1}, left{1}, right{m};
    while(left <= right) {
        ll mid = left + (right-left)/2;
        if(valid(mid)) right=mid-1, ans=mid;
        else           left=mid+1;
    }
    cout << ans;
}
