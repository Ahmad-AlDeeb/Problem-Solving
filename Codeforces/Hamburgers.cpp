...
ll r;
bool valid(ll x) {
    ll bread = (B*x)-b; bread=max(bread,0LL);
    ll sus = (S*x)-s; sus=max(sus,0LL);
    ll cheese = (C*x)-c; cheese=max(cheese,0LL);

    ll res = bread*pb + sus*ps + cheese*pc;
    return r >= res;
}
void fast() {
    ...
    ll ans{}, left{1}, right{10000000000000};
    while(left <= right) {
