...
void fast() {
    ll a,b,n; cin >> a >> b >> n;
    vector<ll> a_div = get_divisors(a); sort(all(a_div));
    vector<ll> b_div = get_divisors(b); sort(all(b_div));

    vector<ll> common;
    set_intersection(a_div.begin(),a_div.end(),b_div.begin(),b_div.end(), common.begin());
    ...
    while(n--) {
        ...
        ll i = common.size()-1;
        for(; i>=0; i--)
            if(common[i]>=l and common[i]<=r)
                {cool=true; break;}

        if(cool) cout << common[i];
        else cout << -1;
        cout << endl;
    }
