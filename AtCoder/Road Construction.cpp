...
void dfs(ll node) {
    ...
    for(ll i{1}; i<=n; i++)
        if(!vis[i] && !binary_search(all(bad_adj[node]),i)) {
            ans.emplace_back(node,i);
        }
}
...
void fast() {
    ...
    for (ll i{1}; i<=n; i++) {
        if(!bad_adj[i].empty()) continue;
        vis[i]=true;

        for (ll j{1}; j<=n; j++)
            if (i != j) {
                if (!vis[j] && !binary_search(all(bad_adj[i]), j)) {
                    vis[j] = true;
