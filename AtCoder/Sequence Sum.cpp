...
void fast() {
    ...
    ll sum{},sub_sum{};
    for(ll i{}; i<res.size(); i++) {
        sum += res[i];
        if(i>=start) sub_sum += res[i];
    }

    ll size = res.size()-start;
    ll remaining = n-res.size();
    ll div = remaining/size;
    sum += div*sub_sum;

    if(remaining%size != 0) {
        ll rem = remaining%size;
        for(ll i{start}; rem--; i++)
            sum += res[i]; // sum%=m;
    }
